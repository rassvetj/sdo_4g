...
<table id="marksheet" class="main-grid" cellspacing="0" data-schedules="<?php echo $totalSchedules;?>" data-persons="<?php echo $totalPersons;?>">
    ...
    <tbody>
        ...
        >
			...
			<?php else: ?>
			<td class="score-cell lesson-cell <?=(!isset($this->scores[$key.'_'.$schedule->SHEID])) ? ('no-score') : ('')?> " <?=($isMarkBrs) ? ('style="background-color:'.$schedule->getColor().'"') : ('');?> >
			<div>
            <?php if (isset($this->scores[$key.'_'.$schedule->SHEID])):?>
                <?php 
			...
			<td class="score-cell total-cell last-cell"><div class="total-ball">
				<?=(isset($this->dataRatingTotal[$key]) ) ? ( round($this->dataRatingTotal[$key]) ):('');?>
			</div></td>
			
			<td class="score-cell total-cell last-cell"><div class="total-ball total-ball-text">
			<?php			
				if($this->scores[$key.'_total']['mark'] <= 0){
					echo 'н/я';
				} else {
