...
<div class="filter_wrap <?php if (!count($this->persons)):?>filter_wrap_nodata<?php endif; ?>">	
	<div class="dateFilterGroup classFWrap">
		<div class="filter_desc">Фильтр по группе/подгруппе:</div>
...
<?php if (!count($this->persons)):?>
    ...
    <div style="padding:10px;text-align:center;color:brown;font-size:14px;">Отсутствуют данные для отображения</div>
<?php else:?>
<?php
...
$issetFailModule 	= false; # Флаг - есть ли модули, включая главный, по которому "Итоговый текущий рейтинг" не сдан (не преодолен порог в 65%)
$isMarkBrs 			= ($this->subject->mark_type == HM_Mark_StrategyFactory::MARK_BRS) ? true : false;
?>

<form id="marksheet-form" method="POST" action="<?=$this->escape( $this->url(array('module' =>'marksheet', 'controller' => 'index', 'action' => 'set-score')) );?>">
<table id="marksheet" class="main-grid" cellspacing="0" data-schedules="<?=$totalSchedules;?>" data-persons="<?=$totalPersons;?>">
    <colgroup><col><col><col span="<?=$totalSchedules + 1;?>"></colgroup>
    <thead>
        <tr class="marksheet-labels">
            ...
            <?php foreach($this->schedules as $key => $schedule):?>            
				<?php
				$isJPractice = ($schedule->typeID == HM_Event_EventModel::TYPE_JOURNAL_PRACTICE) ? (true) : (false);
				$subject_id  = $schedule->CID;
				?>
            <td class="lesson-cell score-cell">
                <?=($schedule->getResultsUrl()) ? ('<a title="Подробная статистика" href="'.$schedule->getResultsUrl().'">') : ('');?>
					<div class="hm-marksheet-table-header-item-title" title="<?=$this->escape($schedule->title)?>"><?=$this->escape($schedule->title)?></div>					
                <?=($schedule->getResultsUrl()) ? ('</a>') : ('');?>
			...
			<td class="score-cell total-cell last-cell"><span class="total-score-label"><?php echo _("Причины недопуска по главному модулю") ?></span></td>
			<td class="score-cell total-cell last-cell"><span class="total-score-label"><?php echo _("Итоговый текущий рейтинг по главному модулю") ?></span></td>
			
			
			<?php if(!empty($this->moduleData['subjects'])):  ?>
				<?php foreach($this->moduleData['subjects'] as $subject_id => $subject_name): ?>
					<?php if($subject_id == $this->subject->subid) { continue; } ?>
					<td class="score-cell total-cell last-cell"><span class="total-score-label"><?php echo _("Итоговый текущий рейтинг по доп. модулю:") ?>
				...
				<?php endforeach; ?>
			<?php endif; ?>			
			<td class="score-cell total-cell last-cell"><span class="total-score-label"><?php echo _("Интегральный текущий рейтинг") ?></span></td>
        ...
        <tr class="marksheet-head">
			...
			<?php
				if(!empty($this->moduleData['subjects'])){
					foreach($this->moduleData['subjects'] as $subject_id => $subject_name){
						if($subject_id == $this->subject->subid) { continue; }
						echo '<td class="total-cell last-cell score-cell">&nbsp;</td>';
					}
				}
			?>
    ...
    <tbody>
		...
		<?php
			$isPassAllModule 	 = $this->moduleData['isPassAllModule'][$person->MID];			
        ...
        >
			...
			if(!empty($this->moduleData['subjects'])){
				foreach($this->moduleData['subjects'] as $subject_id => $subject_name) {
					...
					$mod_maxBallMediumRating 	= $this->moduleData['additional'][$subject_id]['maxBallMediumRating'];
					$mod_ratingMedium		 	= $this->moduleData['rating'][$subject_id]['medium'][$person->MID];
					$mod_isDO				 	= $this->moduleData['additional'][$subject_id]['isDO'];
					$isPassMediumRating_module  = Zend_Registry::get('serviceContainer')->getService('Lesson')->isPassMediumRating($mod_maxBallMediumRating, $mod_ratingMedium, $mod_isDO);					
					$fail_class_PassMediumRating_module = ($isPassMediumRating_module) ? '' : 'fail-lesson';					
					
					if($isHasFailInModule === false && $this->moduleData['additional'][$subject_id]['is_fail_module'][$person->MID]){
						$isHasFailInModule = true;
					}
					
					# Есть причины недопуска в доп. модуле: рубежный контроль или общая сумма итогового текущего рейт.
					$fail_class_PassMediumRating_module = ($this->moduleData['additional'][$subject_id]['is_fail_module'][$person->MID]) ? 'fail-lesson' : $fail_class_PassMediumRating_module;
					
					?><td class="score-cell total-cell last-cell <?=$fail_class_PassMediumRating_module;?>">
						<?/*<div class="total-ball"><?=($this->moduleData['rating'][$subject_id]['medium'][$person->MID] >= 0 && $isPassMediumRating_module) ? round($this->moduleData['rating'][$subject_id]['medium'][$person->MID]) : ('');?></div>*/?>
						<?=(empty($fail_class_PassMediumRating_module) ? '' : '<div class="not-pass">')?>
							<div class="total-ball "><span><?=($this->moduleData['rating'][$subject_id]['medium'][$person->MID] >= 0) ? round($this->moduleData['rating'][$subject_id]['medium'][$person->MID]) : ('');?></span></div>
						<?=(empty($fail_class_PassMediumRating_module) ? '' : '</div>')?>						
					</td><?php
				}
			}
			...
			<td class="score-cell total-cell last-cell"><div class="total-ball">			
				<?#=($isPassAllModule) ? round($this->moduleData['integrate'][$key]['medium']) : ('');?>
				<?=round($this->moduleData['integrate'][$key]['medium']);?>
			</div></td>
			
			<td class="score-cell total-cell last-cell"><div class="total-ball">
				<?#=(isset($this->dataRatingTotal[$key]) && $isPassAllModule && $isPassMediumRating && $isPassTotalRating && $show_total_practic && empty( $this->scores[$key.'_total']['fail_message'] ) ) ? ( round($this->dataRatingTotal[$key]) ) : ('');?>
				<?=(isset($this->dataRatingTotal[$key]) && empty( $this->scores[$key.'_total']['fail_message'] ) ) ? ( round($this->dataRatingTotal[$key]) ) : ('');?>
			</div></td>
			
			<td class="score-cell total-cell last-cell">
				<div class="total-ball">
				<?php 
					$total = round($this->moduleData['integrate'][$key]['medium']);
					
					if(empty( $this->scores[$key.'_total']['fail_message'] )){
						$total += round($this->dataRatingTotal[$key]);
					}
				?>
				<?php #if($isPassAllModule && $isPassTotalRating && $show_total_practic) : ?>
					<?=$this->score(array(
							'score' 	=> $total,
							'user_id' 	=> $key,
							'lesson_id' => 'total',
							'scale_id' 	=> $this->subject->getScale($this->subject->mark_type), 
							'mode' 		=> HM_View_Helper_Score::MODE_MARKSHEET,
							'tabindex' 	=> $temp . '00' . $temp1,
							'mark_type' => $this->subject->mark_type
						));					
			...
			<td class="score-cell total-cell last-cell">
				<div class="total-ball total-ball-text">            
					<?php
						if(
						...
						} else{ #if($isPassAllModule && $isPassTotalRating && $show_total_practic) {
...
$( ".btn-grad-vedomost-single" ).click(function() {
	$('#dialog-grad-vedomost-single').attr('data-url', $(this).data('url') );
