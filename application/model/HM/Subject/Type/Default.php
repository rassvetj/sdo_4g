...
class HM_Subject_Type_Default
{
	...
	{	
		...
		foreach($lessons as $lesson){
			...
			if($issetIPZ === false){
				# Если это ИПЗ
				if($this->getService('Lesson')->isTotalPractic($lesson->title)){
					$issetIPZ = true;
				}
			} 
			...
			if($lesson->typeID == HM_Event_EventModel::TYPE_TASK && (stristr($lesson->title, 'задание') !== FALSE)){ 
				$taskMax += $lesson->max_ball;
			}
		...
		$taskRating = $this->getService('Lesson')->normalizeTask($taskRating, $issetIPZ, $taskMax);
			#var_dump($mark_current);
			#var_dump($taskRating);
			#var_dump($taskRating);
			#var_dump($academRating);			
