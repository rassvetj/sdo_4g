...
class HM_User_UserModel extends HM_Model_Abstract
{
	...
	const TYPE_EXPORT_BASE		 = 1; #Выгрузка из СДО в 1С
	const TYPE_EXPORT_DEBT 		 = 2; #Выгрузка из СДО в 1С: должники
	const TYPE_EXPORT_DEBT_TUTOR = 3; #Долг за преподавателями
...
//         }
	...
	/**
	 * Возвращает список доступных типов отчета
	 * @return array
	 */
	public static function getTypeExport(){
		return array(
			self::TYPE_EXPORT_BASE 		 => _('Выгрузка из СДО в 1С'),
			self::TYPE_EXPORT_DEBT 		 => _('Выгрузка из СДО в 1С: должники'),
			self::TYPE_EXPORT_DEBT_TUTOR => _('Долг за преподавателями'),
		);
	}
