    ...
    public function newAction() {
        ...
        if ($this->_request->isPost()) {
            if ($form->isValid($this->_request->getParams())) {
                ...
                if (null !== $form->getValue('lastname_en')) {
                    $array+= array('LastNameLat' => $form->getValue('lastname_en'));
                }

                if (null !== $form->getValue('firstname_en')) {
                    $array+= array('FirstNameLat' => $form->getValue('firstname_en'));
                }
        ...
        if ($this->_request->isPost()) {
            ...
            if ($form->isValid($this->_request->getParams())) {
                ...
                $formValuesArray = array (
                   ...
                   'gender' => 'gender',
                   'lastname_en' => 'LastNameLat',
                   'firstname_en' => 'FirstNameLat',
                   'tel' => 'Phone',
                   'tel2' => 'CellularNumber',
				...
				$array['begin_learning'] = $this->validateDate($array['begin_learning']);
...
//                     $user = $this->getService('User')->update($user->getValues());
                ...
                */
                ...
                $arr = array(
                    ...
                    'firstname_en' => $user->FirstNameLat,
                    'lastname_en' => $user->LastNameLat,
                    'email' => $user->EMail,
                    'status' => $user->blocked,
                    'user_id' => $userId,
			...
			if(empty($user->begin_learning)){
				$arr['begin_learning'] = '';
			} else {
				$begin_learning 		= new HM_Date($user->begin_learning);
				$arr['begin_learning']	= $begin_learning->toString(Zend_Date::DAY.'.'.Zend_Date::MONTH.'.'.Zend_Date::YEAR);				
			}
...
//             }
	...
	public function validateDate($raw)
	{
		if(empty($raw)){ 
			return new Zend_Db_Expr("NULL"); # обновляем поле на NULL
		}
				
		try {
			$begin_learning = new HM_Date($raw);
			$year			= (int)$begin_learning->toString(Zend_Date::YEAR);
			
			if($year > 9999){ return NULL; } # не обновляем поле
				
			return $begin_learning->toString(Zend_Date::YEAR.'-'.Zend_Date::MONTH.'-'.Zend_Date::DAY);
		
		} catch (Exception $e) {
			return NULL; # не обновляем поле
		}
		return NULL;
	}
