...
class User_ExportController extends HM_Controller_Action
{
    ...
    public function getCsvAction() {
		...
		$select2->group($fieldsGroup);
			...
			#$select2->where('session.subid IN (4838, 4924, 12572)');
			#$select2->where('s.MID = 31561');			
		...
		$sub_message->where($userService->quoteInto('session.subid IN (?)', $sessionIDs));
		
		#echo $sub_message->assemble();
		#die;
