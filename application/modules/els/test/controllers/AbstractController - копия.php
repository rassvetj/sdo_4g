...
//             }
    ...
    public function indexAction()
    {
        ...
        if ($subjectId) {
            ...
            $select->from(
                ...
                array('t.test_id', 't.title', 't.location', 'subject' => 's.subject_id',
            ...
            $select->from(
                array('t' => 'test_abstract'),
                array('t.test_id','t.title', 't.questions', 'public' => 't.status','statustemp' => 't.status',
        ...
        $grid->updateColumn('title',
            array('callback' =>
                array('function' =>
                    ...
                    'params'   => array('{{title}}', '{{status}}', '{{subject}}', '{{test_id}}')
                )
            )
        );
    ...
    public function updateName($name, $location, $subjectId, $testId)
    {
        $subject_id = $this->_getParam('subject_id', 0);
