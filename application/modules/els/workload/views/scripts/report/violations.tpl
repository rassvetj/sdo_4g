...
<?php $this->headScript()->captureStart() ?>
	...
	function sendForm(){		
		var data   = $('#form-area form').serialize();		
		$('#report-area').html('<p><?=_('Отчет формируется')?>...</p>');
		$.ajax(<?= Zend_Json::encode( $this->baseUrl($this->url(array('controller' => 'violations', 'action' => 'get-violations-report'))) ) ?>, {
			type: 'POST',
			global: false,
			data: data
		}).done(function (data) {		
