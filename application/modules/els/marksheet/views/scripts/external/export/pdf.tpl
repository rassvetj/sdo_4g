<?php 		
	...
	$commission_members_html 	= array();
	$commission_members_html[1] = '_________________________ / _________________________ /';
	...
	if(!empty($this->marksheet->commission_members)){
		...
		foreach($this->marksheet->commission_members as $member){
			...
			if($member->confirm){
				$row =  '__подтверждено ' . date('d.m.Y', strtotime($member->confirm->date_created)) . '__' . $row;
			} else {
				$row =  '_________________________' . $row;
			}
			$commission_members_html[$count] = $row;
			
			$commission_members[$count] = $member->getName();
...
<table style="text-align: center; width:100%" >
	...
	<tr><td style="font-weight: bold;">ЗАЧЕТНО-ЭКЗАМЕНАЦИОННАЯ ВЕДОМОСТЬ № <?=$this->marksheet->external_id?></td></tr>
	
	<tr><td style="font-size: 14px; text-align: left;">
		<span style="text-indent: 15px;">Форма обучения	<span style="text-decoration: underline;"><?=$this->marksheet->form_study?></span></span>
		<span style="text-indent: 15px;">Группа  <span style="text-decoration: underline;"><?=$this->marksheet->group_name;?></span></span>
		<span style="text-indent: 15px;"><span style="text-decoration: underline;"><?=$this->marksheet->semester?>      семестр (<?=(!empty($this->marksheet->course))?$this->marksheet->course:'        '?> курс)</span></span>
		<span style="text-indent: 15px;">Учебный год <span style="text-decoration: underline;"><?=$this->marksheet->year?></span></span>
		<br>
		<span style="text-indent: 15px;">Дисциплина <span style="text-decoration: underline;"><?=$this->subject->name;?> (<?=$this->subject->exam_type_name?>)</span></span>
		<span style="text-indent: 15px;">   Номер попытки <span style="text-decoration: underline;"><?=$this->marksheet->attempt;?></span></span>
		<br>
		<span style="text-indent: 15px;">Ф.И.О. экзаменаторов <span style="text-decoration: underline;">
			<?=implode(', ', $commission_members)?>
		</span></span>
	</td></tr>		
</table>
...
<table style="text-align: center; width:100%; font-size: 14px;" class="tbl-content">
	...
	<tbody>
		...
		<?php foreach($this->students as $student): ?>
			...
			<tr >
				<td><?=$row_number;?>.</td>
				<td style="text-align:left"><?=$this->escape($student->getName());?></td>
				<td><?=$student->recordBookNumber;?></td>
				<td><?=$student->marks->mark_current;?></td> 
				<td></td>
				<td><?=$student->marks->mark_landmark;?></td>
				<td><?=$student->marks->mark;?></td>
				<td style="padding:0px; margin: 0px; width:20px" cellpadding="0">
					<?php			
					if(!isset($group_marks[$student->marks->ball])){
						$group_marks[$student->marks->ball] = 1;	
					} else {
						$group_marks[$student->marks->ball]++;
					}				
					?>
					<?=$student->marks->ball;?>				
				</td>
...
<table style="width:100%; padding-top:25px" class="tbl-footer">
	...
	<tr>
        <td>"зачтено" <span style="text-decoration: underline;"><?=(int)$group_marks['зачтено']?></span></td> <td><?=$signerCaption?></td> <td style="height: 15px;" nowrap>_________________________ / <?=($this->marksheet->dean)?$this->marksheet->dean:'_________________________'?> /</td>
	</tr>
	<tr>
		<td>"не зачтено" <span style="text-decoration: underline;"><?=(int)$group_marks['не зачтено']?></span></td> <td>Дата <?=date('d.m.Y', strtotime($this->marksheet->date_issue))?></td> <td></td>
	</tr>
	<tr>
		<td>"отлично" <span style="text-decoration: underline;"><?=(int)$group_marks['отлично']?></span></td> <td>Экзаменаторы (п. 9)</td> <td style="height: 15px;" nowrap><?=$commission_members_html[1]?></td>
	</tr>
	<tr>
		<td>"хорошо" <span style="text-decoration: underline;"><?=(int)$group_marks['хорошо']?></span></td> <td></td> <td style="height: 15px;" nowrap><?=$commission_members_html[2]?></td>
	</tr>
	<tr>
		<td>"удовлетворительно" <span style="text-decoration: underline;"><?=(int)$group_marks['удовлетворительно']?></span></td> <td></td> <td style="height: 15px;" nowrap><?=$commission_members_html[3]?></td>
	</tr>
	<tr>
		<td>"неудовлетворительно" <span style="text-decoration: underline;"><?=(int)$group_marks['неуд.']?></span></td> <td></td> <td style="height: 15px;" nowrap><?=$commission_members_html[4]?></td>
	</tr>
	<tr>
		<td>"не явились" <span style="text-decoration: underline;"><?=(int)$group_marks['неявка']?></span></td> <td></td> <td style="height: 15px;" nowrap><?=$commission_members_html[5]?></td>
	</tr>
</table>
<br>
