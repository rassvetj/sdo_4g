        ...
        xhr = $.ajax($this.attr('href'), {
            ...
            if (textStatus != "abort") {
                ...
                if (xhr.status == 404 || xhr.status == 500) {
                    ...
                    $.ui.errorbox.clear();
                    $('<div>' + <?= Zend_Json::encode($pageMsg->errMsgsLoading); ?> + '</div>').errorbox({ level: 'error' });
                }
            }
        }).done(function (data) {
    ...
    xhr = $.ajax(action, {
        ...
        if (textStatus != "abort") {
            ...
            if (xhr.status == 400) {
                ...
                elsHelpers.alert(<?= Zend_Json::encode($pageMsg->errNewSectnion) ?>, <?= Zend_Json::encode(_("Ошибка при создании темы")) ?>);
            }
        }
    }).done(function (data) {
        var $data = $(data)
...
$(document.body).delegate('.forum-index .topics-list-container .topic-delete', 'click.forum2', function (event) {
    ...
    elsHelpers.confirm(<?= Zend_Json::encode(_('Вы действительно хотите удалить категорию?')) ?>, <?= Zend_Json::encode(_('Удаление категории')) ?>).done(function () {
        $.get($target.attr('href')).always(function () {
            $.ui.errorbox.clear($target.parent());
        }).done(function () {
            $('<div>').insertAfter($target)
                .text(<?= Zend_Json::encode(_('Категория была удалена')) ?>).errorbox();
            $('<a>link</a>')
                .insertAfter($target)
                .addClass('visuallyhidden')
            ...
            $('<div>').insertAfter($target)
                .text(<?= Zend_Json::encode(_('Ошибка при удалении категории')) ?>).errorbox();
        });
    });
})
