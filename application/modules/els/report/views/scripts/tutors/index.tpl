	...
	function getReport(tutor_id) {
		...
		$.ajax(<?= Zend_Json::encode( $this->baseUrl($this->url(array('module' => 'report', 'controller' => 'tutors', 'action' => 'get'))) ) ?>, {
				...
				_.defer(function () {
					...
					$('#' + contentID).html('<b><?=_('Сокращения:')?></b>&nbsp;<b><?=_('т.')?></b><?=_(' - тьютор,')?>&nbsp;<b><?=_('ст.')?></b><?=_(' - студент,')?>&nbsp;<b><?=_('сообщ.')?></b><?=_(' - сообщение.')?>' + data);
...
<script>
    $(document).ready(function(){
		...
		tutors.on('change', function(){
			...
			if(tutorVal == -1){
				var contentID = 'content-area';		
				$('#' + contentID).html('<div class="description-area-tutor"><?=_('Выберите из списка')?></div>');
			} else {
				getReport(tutorVal);				
			}
        });
