						...
						<?php foreach($this->dayList as $key => $day): ?>
							...
							<td class="ball_row_<?=$key?>">
								<?php 																	
									...
									$ball_user_value_normal 	= round($this->ballWeightPractic * $ball_user_value, 2);									
								?>
								<div class="mode_base">
									<div class="<?=($ball_user_value_normal > 0) ? 'score_red' : 'score_gray'; ?> number_number">
										<span align="center"><?=($ball_user_value_normal > 0) ? round($ball_user_value_normal, 2) : 'Нет'; ?></span>
									</div>
								</div>															
							</td>							
						<?php endforeach; ?>																
						<td>							
							<div class="<?=($this->lessonResult->ball_academic > 0) ? 'score_red' : 'score_gray'; ?> number_number">
								<span align="center"><?=($this->lessonResult->ball_academic > 0) ? round($this->lessonResult->ball_academic, 2) : 'Нет'; ?></span>
							</div>
						</td>
						<td>
							<div class="<?=($this->lessonResult->ball_practic > 0) ? 'score_red' : 'score_gray'; ?> number_number">
								<span align="center"><?=($this->lessonResult->ball_practic > 0) ? round($this->lessonResult->ball_practic, 2) : 'Нет'; ?></span>
							</div>
						</td>
					</tr>						
			</tbody>
