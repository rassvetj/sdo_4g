		...
		<thead>
			<tr>
				...
				<?php foreach($this->dayList as $dayId => $dayCaption): ?>
					...
					<th colspan="2" id="col_day_<?=$dayId?>" style="height: 45px;">
						<div>
				...
				<?php endforeach; ?>
				<th colspan="2" class="new-col hidden">
			...
			<tr>
				<?php foreach($this->dayList as $dayCaption): ?>
					<th>Явка</th>
					<th>Оценка</th>
				<?php endforeach; ?>
				<th class="new-col hidden">Явка</th>
				<th class="new-col hidden">Оценка</th>
				<th>АА</th>
				<th>ПР</th>
			</tr>				
		</thead>
		...
		<?php foreach($this->users as $user) : ?>
			<tr class="odd fio-cell">
				...
				<?php foreach($this->dayList as $dayId => $dayCaption): ?>
					...
					<td class="item-container" data-ceil_key="<?=$ceilKey?>" >
						<?php
						$ballValue       = isset($userData['mark']) ? $userData['mark'] : 0;
						$ballValueNormal = round($this->ballWeightPractic * $ballValue, 2);
						?>
						<input type="hidden" name="mark[<?=$ceilKey?>]" value="<?=$ballValue?>" disabled="disabled" >
						<div class="markCaption">									
							<div class="<?=$ballValueNormal>0 ? 'score_red' : 'score_gray'; ?> number_number">
								<span align="center" id="caption_mark_<?=$ceilKey?>"><?=$ballValueNormal>0 ? $ballValueNormal : 'Нет'?></span>
							</div>
						</div>
					</td>
				<?php endforeach;?>
				<td class="item-container new-col isBe-new-ceil hidden" data-user_id="<?=$user['MID']?>" data-ceil_key="<?=$user['MID']?>_new" >&nbsp;</td>
				<td class="item-container new-col ball-new-ceil hidden" data-user_id="<?=$user['MID']?>" data-ceil_key="<?=$user['MID']?>_new">&nbsp;</td>
				
				<td>
					<?php $aa_ball = Zend_Registry::get('serviceContainer')->getService('Lesson')->normalizeAcadem($user['ball_academic']); ?>
					<div class="<?=$aa_ball>0 ? 'score_red' : 'score_gray'; ?> number_number">
						<span align="center"><?=$aa_ball>0 ? round($aa_ball, 2) : 'Нет'; ?></span>
					</div>
				</td>
				<td>
					<div class="<?=($user['ball_practic'] > 0) ? 'score_red' : 'score_gray'; ?> number_number">
						<span align="center"><?=($user['ball_practic'] > 0) ? round($user['ball_practic'], 2) : 'Нет'; ?></span>
					</div>
				</td>
			</tr>
		<?php endforeach;?>
