...
class Poll_ListController extends HM_Controller_Action_Crud
{
    ...
    {
        $this->_setForm(new HM_Form_Poll());
        parent::init();
    ...
    {
        return array(
            self::ACTION_INSERT => _('Опрос успешно создан'),
            self::ACTION_UPDATE => _('Опрос успешно обновлён'),
            self::ACTION_DELETE => _('Опрос успешно удалён'),
            self::ACTION_DELETE_BY => _('Опросы успешно удалены')
        );
    }
    ...
    public function indexAction()
    {
        ...
        if(in_array($this->getService('User')->getCurrentUserRole(), $rolesWithFilter)){
            $filters['public'] = array('values' => HM_Poll_PollModel::getStatuses());
        ...
        $group = array(
            't.quiz_id', 't.title', 't.location',
            't.questions',
            't.updated', 't.quiz_id', 't.status'
        );
        ...
        if ($subjectId) {
            ...
            $select->from(
                    array('t' => 'quizzes'),
                    array('t.quiz_id', 't.title', 't.location',
                        'subject' => 's.subject_id', 't.questions',
                        't.updated', 'tags' => 't.quiz_id',
            ...
            $select->joinLeft(
                   ...
                   ->where('(t.location = ' . (int) HM_Poll_PollModel::LOCALE_TYPE_GLOBAL . ' AND t.status = ' . (int) HM_Poll_PollModel::STATUS_STUDYONLY . ') OR t.subject_id = ' . (int) $subjectId);
            ...
            $select->from(
                array('t' => 'quizzes'),
                array('t.quiz_id', 't.title', 't.questions', 'public' => 't.status',
                    't.updated','tags' => 't.quiz_id',
                    'classifiers' => new Zend_Db_Expr("GROUP_CONCAT(cf.name)"),
                )
            )
        ...
        $select->joinLeft(
            ...
            'cl.item_id = t.quiz_id AND cl.type = ' . HM_Classifier_Link_LinkModel::TYPE_POLL,
            array()
        );
        $select->joinLeft(
                    ...
                    $subjectId ? 
                        array(
                            ...
                            'callback' => array(
                                ...
                                'params' => array(HM_Event_EventModel::TYPE_POLL, '{{quiz_id}}', '{{subject}}', $subjectId)
                            )) : 
                        array('hidden' => true)
                ),
        ...
        if ($subjectId) {
            ...
            $options = array(
                    ...
                    'global' => array('name' => 'global', 'title' => _('все, включая опросы из Базы знаний'), 'params' => array('subject' => null), 'order' => 'subject', 'order_dir' => 'DESC'),
            );
        ...
        if($subjectId > 0){
            ...
            $grid->addMassAction(
                array('module' => 'poll', 'controller' => 'list', 'action' => 'unpublish'),
                _('Назначить статус: не опубликован')
            );
        }

//        if(in_array($this->getService('User')->getCurrentUserRole(), array(HM_Role_RoleModelAbstract::ROLE_MANAGER,HM_Role_RoleModelAbstract::ROLE_DEVELOPER))){
            $grid->addMassAction(
...
//        }
        ...
        $grid->updateColumn('tags', array(
                'callback' => array(
                    ...
                    'params'=> array('{{tags}}', $this->getService('TagRef')->getPollType(), $subjectId, '{{locationtemp}}' )
                )
            ));
    ...
    public function create(Zend_Form $form)
    {
        ...
        if ($tags = $form->getParam('tags')) {
            $this->getService('Tag')->update( $tags, $quiz->quiz_id, $this->getService('TagRef')->getPollType() );
        }
...
/*        if(!$quiz->isEditAllowed($subjectid, $userRole)){
        ...
        $this->getService('Tag')->update( $form->getParam('tags',array()), $this->_request->getParam('quiz_id'), $this->getService('TagRef')->getPollType() );
    ...
    public function unassignAction()
    {
        ...
        if (strlen($postMassIds)) {
            ...
            if (count($ids)) {
                foreach($ids as $id) {
                    $this->getService('Poll')->clearLesson($this->id, $id);
    ...
    public function updateActions($status, $subjectId, $actions)
    {
        ...
        if($this->getService('Poll')->isEditable($subjectId, $subject_id, $status)){
            ...
            //оставялем только "просмотреть"
            $result     = '';
            $actionPart = explode('</li>', $actions);
            if (count($actionPart) > 2) {
                $result    = implode('</li>', array($actionPart[0], $actionPart[count($actionPart)-1]));
            }
            return $result;
        }
    }
    ...
    {
        $statuses = HM_Poll_PollModel::getStatuses();
        return $statuses[$status];
    ...
    public function unpublishAction()
    {
        ...
        if (strlen($postMassIds)) {
            ...
            if (count($ids)) {
                foreach($ids as $id) {
                    $this->getService('Poll')->unpublish($id);
                }
            }
    ...
    {
        if (!empty($questions)) { //&& $this->getService('User')->getCurrentUserRole() == HM_Role_RoleModelAbstract::ROLE_DEAN) {
