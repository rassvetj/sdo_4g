function piece(pparent,pi,ptitle,pcolor,pvalue)
{
	...
	{
		var input = $(thisPiece).find(strInputClassName)
		var me = this
		input
		.keypress(function(e){
			if (e.keyCode==13)
		...
		/*
		.focus(function(){
			$(me.block).find('.current').show()
		})
		.blur(function(){
			$(me.block).find('.current').hide()
		})
		*/
	}
	...
	var updateInput = function()
	{
		...
		if (typeof jqInput == "undefined")
			jqInput = $(parent.getContainer()).find(strInputClassName)
	...
	this.setValueByDelta = function(d)
	{
		...
		var oldValue = value
		this.setValue(value+d)
		var newValue = value
		
		$(this.block).width(this.getWidth())
		var cl = parseInt($(this.block).left(),10)
		
		var newDelta = ((newValue - oldValue) * parent.getPieWidth()) / 100
		
		$(this.block).left(cl - newDelta)
		//$(this.block).animate({left:cl - newDelta},'normal')
	}
	...
	this.setWidthByValue = function(newValue)
	{
		...
		var upperBound = value+nextValue
		newValue = Math.min(Math.max(0,newValue),upperBound)
		
		
		next.setValueByDelta(-newValue + value)
		this.setValue(newValue)
		
		var nw = Math.round(value * parent.getPieWidth() / 100)
		myDiv.style.width = nw + 'px'
		$(myDiv).animate({width: nw},'normal')
	}
		...
		$(this.block)
			...
			.mousedown(function(e){
				...
				*/
				...
				{
					myPreviousDiv.e = e
					$(myPreviousDiv).find('.resize').mousedown()
					return
				}
				*/
...
function pie()
{
	...
	{
		if ($(document.body).find('div#alt').size() > 0)
			return
	...
	*/
	...
	this.drawIn = function(w)
	{
		...
		// add captions
		$(where).append('<div class="captions">'+caps+'</div>')
		...
		// add eq button
		$(where).append('<input value="Сделать равными" type="submit" class="send" onclick="'+myName+'.makeEqual()" />')
		// add submit button
		$(where).append('<input value="Отправить" type="submit" class="eq" onclick="'+myName+'.send()" />')
	}
