...
Element.Methods = {
  ...
  ancestors: function(element) {
    return $P(element).recursivelyCollect('parentNode');
  },

  descendants: function(element) {
    return $P(element).select("*");
  },
  ...
  immediateDescendants: function(element) {
    if (!(element = $P(element).firstChild)) return [];
    while (element && element.nodeType != 1) element = element.nextSibling;
    if (element) return [element].concat($P(element).nextSiblings());
    return [];
  },

  previousSiblings: function(element) {
    return $P(element).recursivelyCollect('previousSibling');
  },

  nextSiblings: function(element) {
    return $P(element).recursivelyCollect('nextSibling');
  },
  ...
  match: function(element, selector) {
    ...
    return selector.match($P(element));
  },
  ...
  previous: function(element, expression, index) {
    element = $P(element);
    if (arguments.length == 1) return $P(Selector.handlers.previousElementSibling(element));
  ...
  next: function(element, expression, index) {
    element = $P(element);
    if (arguments.length == 1) return $P(Selector.handlers.nextElementSibling(element));
  ...
  select: function() {
    var args = $A(arguments), element = $P(args.shift());
  ...
  identify: function(element) {
    ...
    if (id) return id;
    do { id = 'anonymous_element_' + self.counter++ } while ($P(id));
  ...
  getHeight: function(element) {
    return $P(element).getDimensions().height;
  },

  getWidth: function(element) {
    return $P(element).getDimensions().width;
  },
  ...
  getOffsetParent: function(element) {
    if (element.offsetParent) return $P(element.offsetParent);
    if (element == document.body) return $P(element);
...
if (Prototype.Browser.Opera) {
  ...
  Element.Methods.getOffsetParent = Element.Methods.getOffsetParent.wrap(
    function(proceed, element) {
      ...
      catch(e) { return $P(document.body) }
  ...
  findElements: function(root) {
    ...
    switch (this.mode) {
      case 'selectorsAPI':
        ...
        if (root !== document) {
          var oldId = root.id, id = $P(root).identify();
          e = "#" + id + " " + e;
        }
...
/^:((first|last|nth|nth-last|only)(-child|-of-type)|empty|checked|(en|dis)abled|not)(\((.*?)\))?(\b|$|(?=\s|[:+~>]))/,
  ...
  handlers: {
    ...
    id: function(nodes, root, id, combinator) {
      var targetNode = $P(id), h = Selector.handlers;
  ...
  matchElements: function(elements, expression) {
    var matches = $$P(expression), h = Selector.handlers;
    h.mark(matches);
...
Form.Methods = {
  ...
  getElements: function(form) {
    return $A($P(form).getElementsByTagName('*')).inject([],
...
(function() {
  ...
  if (document.addEventListener) {
    ...
    $P("__onDOMContentLoaded").onreadystatechange = function() {
