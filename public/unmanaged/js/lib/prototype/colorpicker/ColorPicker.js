...
Refresh.Web.ColorPicker.prototype = {
	...
	initialize: function(id, settings) {
		...
		this._hueRadio = $P(this.id + '_HueRadio');
		this._saturationRadio = $P(this.id + '_SaturationRadio');
		this._valueRadio = $P(this.id + '_BrightnessRadio');
		
		this._redRadio = $P(this.id + '_RedRadio');
		this._greenRadio = $P(this.id + '_GreenRadio');
		this._blueRadio = $P(this.id + '_BlueRadio');
		//this._webSafeCheck = $P(this.id + '_WebSafeCheck');
		...
		this._preview = $P(this.id + '_Preview');
		
		
		// MAP
		this._mapBase = $P(this.id + '_ColorMap');
		...
		// attach map slider
		this._map = new Refresh.Web.Slider(this._mapL2, {xMaxValue: 255, yMinValue: 255, arrowImage: this.settings.clientFilesPath + 'mappoint.gif', size: {width: 15, height: 15}});

		// attach color slider
		this._slider = new Refresh.Web.Slider(this._barL4, {xMinValue: 1,xMaxValue: 1, yMinValue: 255, arrowImage: this.settings.clientFilesPath + 'rangearrows.gif', size: {width: 40, height: 9}});;
