    ...
    */
	...
	this.setConditions = function(xconditions)
	{
		...
		for (var i = 0; i < xconditions.conditions.length; i++)
		{
			...
			if (xid == '1')
			{
				...
				if (n.length > 20)
					n = n.substring(0,20)+'...'
				t = '<?=_("Связать с")?> "<strong>'+n+'</strong>" <?=_("условием")?>&nbsp;&nbsp;<?php echo addslashes($GLOBALS['tooltip']->display('schedule_adaptive_mark'))?>'
			}
			else
			{
	...
	this.showSettings = function()
	{
		...
		jQuery(settingsLayer)
			.find('.wndCaption')
			.html('<?=_("Условия для занятия")?> "<strong title="'+this.getName()+'">'+n+'</strong>"')
    ...
    var dx1Md1 = summerMinus(startDate,parent.getStartDate())
		...
		var itemBlock = jQuery(container).append('<div class="itemBlock"><div class="itemCaption">'+before+'<span class="itemId">'+id+'</span>'+after+'</div></div>')
			.find('.itemBlock')
			.click(function(e){
				if (e.ctrlKey)
				{
					...
					var x = ''
					+'<div class="condition">'
						+'<div class="ctext"><?=_("Связать с")?> "<strong>'+n+'</strong>" <?=_("условием")?>&nbsp;&nbsp;<?php echo addslashes($GLOBALS['tooltip']->display('schedule_adaptive_mark'))?></div>'
...
function timeline()
{
	...
	var generateConditionsBlock = function()
	{
		...
		+'<tr>'
			...
			+'<td>&nbsp;<?=_("И")?>&nbsp;&nbsp;<?php echo addslashes($GLOBALS['tooltip']->display('schedule_adaptive_func'))?></td>'
		+'</tr>'
		+'<tr>'
		...
		all+='<td>&nbsp;<?php echo addslashes($GLOBALS['tooltip']->display('schedule_adaptive_schedule'))?>&nbsp;&nbsp;</td>'
		all+='<td>'+radio+'</td>'
		all+='</tr></table>'
	...
	var createSettingsWindow = function()
	{
		...
		+ '<table cellspacing="0" class="wndSettings">'
			...
			+ '</tr>'
			+ '<tr>'
				+ '<td class="padding" align="left"><a class="ok" href="javascript:void(0)"><?=_("сохранить")?></a></td>'
				+ '<td class="padding" align="right"><a class="cancel" href="javascript:void(0)"><?=_("отмена")?></a></td>'
			+'</tr>'
		+'</table>'
	...
	var drawTracksAndItems = function()
	{
		...
		htmlTL = '<div class="timeline"><table cellspacing="0" cellpadding="0">'+htmlTL+'</table>'+rowNames+'</div>'

		jQuery(container+' .timeline').remove()
		jQuery(container+' .scaleChange').after(htmlTL)
		
		initialRowNamesWidth = $(container+' .floatingRowNames').width()
	...
	var drawCaptions = function(boolDrawDays)
	{
		...
		var aDays = ['<?=_("Вс")?>','<?=_("Пн")?>','<?=_("Вт")?>','<?=_("Ср")?>','<?=_("Чт")?>','<?=_("Пт")?>','<?=_("Сб")?>']
	...
	var initialRowNamesWidth = 0
	var scrollTimeout = null
	function scrollRowNames()
	{
		offset = htmlElement.scrollLeft
		offset = Math.max(0, offset-335)
		$('.floatingRowNames')
			.stop()
			.animate({width: (initialRowNamesWidth + (offset != 0 ? offset + 10 : 0 ))+'px'}, 300)
	}
