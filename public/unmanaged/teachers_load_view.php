...
if(isset($_POST['date'])) {
   ...
   $to = $_POST['date']['to']['Year']."-".$_POST['date']['to']['Month']."-".$_POST['date']['to']['Day']." 23:59:59";
...
//Набиваем массив с двумя индексами mid и sheid а значение количествa часов этого занятия
//если человек не ведет этого занятия то пусто
//и заодно заполняем массив занятий для шаблона smarty
$sheids_array = Schedules::get_all_createID_at_period($mid,$from,$to);
$count_hours = array();
...
//    echo "<pre>";
//    print_r($count_hours);
//    print_r($sum_count_hours);
//    print_r($time_marks_for_smarty);
//    echo "</pre>";
    ...
    $smarty_tpl->assign("g_fr_year",$_POST['date']['from']['Year']);
    $smarty_tpl->assign("g_fr_month",$_POST['date']['from']['Month']);
    $smarty_tpl->assign("g_fr_day",$_POST['date']['from']['Day']);
    $smarty_tpl->assign("g_to_year",$_POST['date']['to']['Year']);
    $smarty_tpl->assign("g_to_month",$_POST['date']['to']['Month']);
    $smarty_tpl->assign("g_to_day",$_POST['date']['to']['Day']);
    $smarty_tpl->assign("g_mid",$_POST['mid']);
    $smarty_tpl->assign("print",getIcon("print"));
    $smarty_tpl->display("individual_teachers_load_view.tpl");
    ...
    $smarty_tpl->assign("g_mid",$_POST['mid']);
    $smarty_tpl->assign("print",getIcon("print"));
    $smarty_tpl->display("teachers_load_view.tpl");
}
?>