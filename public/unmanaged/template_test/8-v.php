...
правильный вариант галочкой. Обязательно "),
   'balcalc'=>'user',
   'qmoder'=>0,
   'goodotvet'=>1,
   'string'=>array(
...
function v_vopros_8(&$vopros,$tm,$number,&$attach) {
// выводит вопрос на экран
   ...
   $kod=$vopros[kod];
   $v_number=$number+1;
   //        $GLOBALS['v_edit_1'][title]."<BR>".
   //$v_text=v_bbparse($kod,$vopros[vopros]);
   $v_text=v_bbparse_vopros($kod,$vopros,$vopros[vopros],$number,$attach);
   //."<table  border=1 width=80% class="brdr">";

   include("$tm-v-top.html");
//   "<table width="80%">";
   //Если есть файл прикреплённый к тексту вопроса, то картинки-ответы идут следом, а нет значит все прикреплённые файлы - ответы
   $v_i = (substr($attach[0]['fname'],0,13) == 'comment_file_') ? 1 : 0;
   
   $arPosQuestion = array();
   // $i = 1 - да, массив $vopros[variant] начинается с единицы
   for($i = 1; $i <= count($vopros[variant]); $i++) $arPosQuestion[] = $i;
   if ($mode === 1) {
		if($vopros['is_shuffled'] == 1) {
           shuffle($arPosQuestion);
		}
   }
   
   foreach ($arPosQuestion as $i => $pos) {
      $k = $pos;
      $v = $vopros[variant][$pos];
      $checked = false;
      if ($vopros['otvets'][0]==$i) $checked = true;

      $v_image=v_bbparse_img($kod,$mode,($v_i === 1 ? $k + 1 : $k));  // графический вариант ответа!!!
...
function v_otvet_8(&$vopros,$tm,$number,&$attach,&$form) {
   /*
   echo "VOPROS";
   pr($vopros);
   echo "FORM";
