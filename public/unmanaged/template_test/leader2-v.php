...
function v_sql2php_leader2(&$vopros) {
...
function v_php2sql_leader2($arr) {
...
function v_vopros_leader2(&$vopros,$tm,$number,&$attach) {
    global $s;
...
function v_otvet_leader2(&$vopros,$tm,$number,&$attach,$form) {
...
/*            if (!isset($form[$k])) {
            ...
            $answers = array();
            $weight_summ = 0;
            foreach($form[$k] as $answer => $checked)
            // передан недопустимый ответ
            if (!isset($vopros['variant2'][$answer])) {
                ...
                $weight_summ += $doklad['weights'][$answer+1];
                $answers[] = $vopros['variant2'][$answer];
            }

            $otv[$k] = $form[$k];
            $doklad['otv'][$k] = $form[$k];
            $doklad['main'][$k] = sprintf(_("По слушателю %s выбран вариант %s"), $vopros[variant1][$k]['fio'], implode(', ', $answers));
            $doklad['good'][$k] = 1;
            $bal += (int) $weight_summ;
        }
    }
