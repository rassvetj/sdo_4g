...
function v_sql2php_leader1(&$vopros) {
...
function v_php2sql_leader1($arr) {
...
function v_vopros_leader1(&$vopros,$tm,$number,&$attach) {
    global $s;   
...
function v_otvet_leader1(&$vopros,$tm,$number,&$attach,$form) {
    ...
    if (is_array($vopros['variant1']) && count($vopros['variant1']) 
        ...
        foreach($vopros['variant1'] as $k=>$v) {
            ...
            $otv[$k] = $form[$k];
            $doklad['otv'][$k] = $form[$k];
            $doklad['main'][$k] = sprintf(_("По слушателю %s выбран вариант %s"),
                                          $vopros[variant1][$k]['fio'],
                                          $vopros['variant2'][$form[$k]]
            );
            $doklad['good'][$k] = 1;
