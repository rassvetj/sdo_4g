...
?>
   <tr align="center">
      <td bgcolor="f5f5f5" width="50px">
        <a href="show.php4?sql=UPDATE+devlog+SET+type='<?=$msg['type']?>',+build='0',+add_date='<?=date("Y-m-d")?>'+WHERE+DiD='<?=$id?>'"><?=$msg['show']?></a>
          </td>
      ...
      <td bgcolor="f5f5f5" width="150px">
<!-- <INPUT type="test" name="file" value="<?=$file?>" readonly> -->
        <?=$file?>
          </td>
      <td bgcolor="f5f5f5" width="50px">
      ...
      <td bgcolor="f5f5f5" width="50px">
        <a href="show.php4?sql=DELETE+FROM+devlog+WHERE+`DiD`='<?=$id?>'">Del</a>
          </td>
      <td bgcolor="f5f5f5" width="50px">
...
<tr align="center">
      ...
      <td bgcolor="#f5f5f5" width="150px"><b>File</b>
          </td>
      <td bgcolor="#f5f5f5" width="50px">
         <b>name</b>
          </td>
...
function dev_sql($type,$param="")
                {
                        ...
                        debug_yes("max",$param->max);
                                switch ($type)
                                        {       case 1 : $sql="SELECT * from devlog WHERE `build`='0' ORDER BY `type` DESC"; break;
                                                case 3 : $sql="SELECT * from devlog WHERE `type`='new' AND `build`='0' ORDER BY `type` DESC"; break;
                                                case 4 : $sql="SELECT * from devlog WHERE `type`='added' AND `build`='0' ORDER BY `type` DESC"; break;
                                                case 5 : $sql="SELECT * from devlog WHERE `type`='return' AND `build`='0' ORDER BY `type` DESC"; break;
                                                case 6 : if (isset($param)) $sql="SELECT * from devlog WHERE `build`='".$param."' ORDER BY `type` DESC"; break;
                                                case 7 : if (isset($param)) $sql="UPDATE devlog SET build='".$param."' WHERE build='0' AND type='added'"; break;
                                                case 8 : if (!empty($param["id"])) $result=@sql("DELETE FROM devlog WHERE `DiD`='".$param["id"]."'"); $sql="INSERT INTO `devlog` (`DiD`, `name`, `email`, `file`, `desc`, `type`, `build`, `post_date`, `add_date`) VALUES ('".$param['id']."', '".$param['name']."', '".$param['email']."', '".$param['file']."', '".$param['desc']."', '".$param['type']."', '".$param['build']."', '".$param['pd']."', '".$param['ad']."')"; break;
                                                case 2 : $sql="SELECT MAX(build) FROM `devlog`"; break;
                                                case 9 : if (isset($param)) $sql="SELECT * from devlog WHERE `DiD`='".$param."'"; break;
