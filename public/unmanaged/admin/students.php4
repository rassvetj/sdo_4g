...
<table align="center" border="0" cellspacing="1" cellpadding="5" width="80%" class=br  style="font-size:13px">
   <tr align="center">
      <td class=questt>
        <b><a href="students.php4"><?=_("Студенты.")?></a></b>
          </td>
   </tr>
  </table>
<?
...
if (isset($edit) && isset($HTTP_GET_VARS["MID"])) {
   $work->complete=0;
   $work->edit=1;
   $work->complete=edit_table("students.php4",$HTTP_GET_VARS["MID"]);
}
...
if (!isset($edit)) {
   $work->complete=0;
   $work->show=1;
   if (isset($order)){$res=sql_query(16,$order);}
   else $res=sql_query(16);
   $work->complete=generate_table("students.php4",$res);
}
