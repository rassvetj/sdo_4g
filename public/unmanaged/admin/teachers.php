...
switch ($_POST['hid_act']) {
        case "add_pmgroup":
                ...
                if (!sqlrows($r)) {
                        $r = sql("INSERT INTO permission_groups (name, type) values ('{$_POST['txt_pmname']}', 'teacher')");
                }
        break;
        case "edit_pmgroup":
                    $r = sql("DELETE FROM permission2act WHERE pmid!='3' AND type = 'teacher'", "err12354");
                	if (is_array($_POST['ch_pmgroups'])) {
                    foreach ($_POST['ch_pmgroups'] as $acid => $arrPmid) {
                            foreach ($arrPmid as $pmid => $blabla) {
                                    $r = sql("INSERT INTO permission2act (acid, pmid, type) values ('{$acid}', '{$pmid}', 'teacher')","err738");
                            }
                    }
                }
        break;
                ...
                if (is_array($_POST['sel_pmgroup'])) {
                      ...
                      WHERE
                        permission2mid.`mid`='{$mid}'";
                                ...
                                $query = "SELECT * FROM permission_groups WHERE type='teacher'";
...
if (isset($_GET['del'])) {
        $intDefaultPmid = getDefaultPmGroup("teacher");
...
<table align="center" border="0" cellspacing="1" cellpadding="5" width="80%" class=br  style="font-size:13px">
        <tr align="center">
            <td class=questt>
                <b><a href="teachers.php"><?=_("Преподаватели")?></a></b>
        </td>
    </tr>
</table>
<br />
...
<br />
<?
$arrTableChecked = array();
$arrTablePermissions = array();
$arrTablePermissionsDefault = array();
$arrTableActions = array();
/*
$q = "          SELECT
                  permission_groups.pmid as pmid_notnull,
                  permission_groups.default as default_group,
                  actions.acid as acid_notnull,
                  permission2act.pmid,
                  permission2act.acid,
                  permission_groups.name as pm_name,
                  actions.name as act_name
                FROM
                  permission2act
                  RIGHT OUTER JOIN actions ON (permission2act.acid = actions.acid)
                  RIGHT OUTER JOIN permission_groups ON (permission2act.pmid = permission_groups.pmid)
                WHERE
                  permission_groups.type = 'teacher' AND
                  actions.type='teacher'";
$q = "SELECT permission_groups.pmid as pmid_notnull,
             permission_groups.default as default_group,
             actions.acid as acid_notnull,
             permission2act.pmid,
             permission2act.acid,
     ...
     FROM permission_groups
          RIGHT JOIN permission2act ON permission2act.pmid = permission2act.pmid
          RIGHT JOIN actions ON permission2act.acid = actions.acid
     WHERE permission_groups.type = 'teacher' AND
           permission2act.type = 'teacher'";

$resIntersect = sql($q,"err344FR");
while ($a = sqlget($resIntersect)) {
        $arrTableChecked[$a['acid_notnull']][$a['pmid_notnull']] = ($a['pmid_notnull'] == $a['pmid'])?"checked":"";
        $arrTablePermissions[$a['pmid_notnull']] = $a['pm_name'];
        $arrTablePermissionsDefault[$a['pmid_notnull']] = $a['default_group'];
        $arrTableActions[$a['acid_notnull']] = $a['act_name'];
}
*/
?>
...
<form name="form_edit" method="post" action="">
        ...
        <?
    			...
    			$arrTablePermissions = get_permissions_all(PERMISSIONS_TEACHER);

                $intNumCols = (count($arrTablePermissions)+1);
                $intWid = intval(100/$intNumCols);
                //var_dump($arrTablePermissions);
                echo "<tr><th>&nbsp;</th>";
                ...
                echo "</tr>";
$i = 0;
$arrTableActions = get_actions(PERMISSIONS_TEACHER);
foreach ($arrTableActions as $acid => $act_name) {
        $act_name = (ord($act_name)<128) ? ((defined($act_name)) ? strtolower(constant($act_name)) : "кафедры") : $act_name;
...
foreach ($arrTablePermissions as $key => $val) {
        ...
        $strDel = (!$arrTablePermissionsDefault[$key]) ? "<a href='{$sitepath}admin/teachers.php?del={$key}'>{$strIcon}</a>" : "";
