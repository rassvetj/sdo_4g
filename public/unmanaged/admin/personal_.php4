<?php

//defines

$include=TRUE ;

// include

require ("setup.inc.php");
//require ("adm_t.php4");
echo show_tb();
require ("adm_fun.php4");


$connect=get_mysql_base();


debug_yes("array",$HTTP_COOKIE_VARS);
?>

<center>
<br><br><br>
<table align="center" border="0" cellspacing="1" cellpadding="5" width="80%" class=br  style="font-size:13px">
   <tr align="center">
      <td class=questt>
        <b><a href="sp.php4"><?=_("Персонал сервера. Тех поддержка")?></a></b>
          </td>
   </tr>
   <tr align="center">
      <td bgcolor="white">
        <b><a href="dekan.php4"><?=_("Персонал сервера. Учебная администрация")?></a></b>
          </td>
   </tr>
   <tr align="center">
      <td class=questt>
        <b><a href="teachers.php4"><?=_("Преподаватели")?></a></b>
          </td>
   </tr>
   <tr align="center">
      <td bgcolor="white">
        <b><a href="students.php4"><?=_("Обучаемые")?></a></b>
          </td>
   </tr>
   <tr align="center">
      <td class=questt>
        <b><a href="abitur.php4"><?=_("Претенденты")?></a></b>
          </td>
   </tr>
   <tr align="center">
      <td bgcolor="white">
        <b><a href="people.php"><?=_("Все учетные записи")?></a></b>
          </td>
   </tr>
   <tr align="center">
      <td class="questt">
        <b><a href="people_add.php"><?=_("Операции с учетными записями")?></a></b>
          </td>
   </tr>
   <tr align="center">
      <td bgcolor="white">
        <b><a href="uplimit.php"><?=_("Несанкционированный доступ")?></a></b>
          </td>
   </tr>
   <tr align="center">
      <td class="questt">
        <b><a href="admin.php4"><?=_("Персонал сервера. Администрация")?></a></b>
          </td>
   </tr>
   <tr align="center">
      <td bgcolor="white">
        <b><a href="log.php4"><?=_("Персонал сервера. LOG-файл регистрации (reg.log)")?></a></b></td>
   </tr>
  </table>
  <br>
 <table align="center" border="0" cellspacing="1" cellpadding="5" width="80%" class=br  style="font-size:13px">
   <tr align="center">
      <td class="questt">
                <b><a href="http://www.hypermethod.com">v.
                <?php
                        $res=sql_query(5);
                        $result=@sqlget($res);
//                      debug_yes("array",$result);
                        echo $result['value'];
                ?>
                </a></b>
          </td>
   </tr>
  </table>


</center>


<?php


//require_once("adm_b.php4");
echo show_tb();
?>