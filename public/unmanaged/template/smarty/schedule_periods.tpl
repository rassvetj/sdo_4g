...
<tr><td align=right>
<table width="0"><tr>
<td><a title="{?t?}Предыдущая неделя{?/t?}" href="{?$sitepath?}schedule_periods.php?tweek={?$tweeklast?}&cid={?$cid?}"><img src="{?$sitepath?}images/icons/left.gif" border="0" /></a></td>
<td>{?$tweeklast|date_format:"%d.%m.%Y"?}</td>
<td> {?$tweek?} </td>
<td>{?$tweeknext|date_format:"%d.%m.%Y"?}</td>
<td><a title="{?t?}Следующая неделя{?/t?}" href="{?$sitepath?}schedule_periods.php?tweek={?$tweeknext?}&cid={?$cid?}"><img src="{?$sitepath?}images/icons/right.gif" border="0" /></a></td>
</tr></table>
</td></tr>
</table>
<br>
...
{?foreach from=$periods item=period?}
   <table width="100%" cellspacing="0" cellpadding="0"  valign="top">
    <tr>
     ...
     <td  align="right" class="th3" nowrap ><b>{?$period.name?}</b>&nbsp;</td>
    </tr>
   </table>
           ...
           <tr>
               ...
               {?if $day_schedule.studies.$i.$j.items?}
	               ...
	               {?foreach from=$day_schedule.studies.$i.$j.items item=study?}
                       ...
                       <a href="schedule.php4?c=delete&rp=periods&sheid={?$study.sheid?}&tweek={?$day_schedule.tweek?}" onClick='return confirm("{?t?}Удалить занятие?{?/t?}")' title='{?t?}Удалить это занятие{?/t?}'>
 ...
 <table width="100%" cellspacing="0" cellpadding="0"  valign="top" >
    <tr>
     <td  align="right" nowrap colspan='3' class="transparent">
      <b><a href='schedule.php4?c=add&period={?$period.lid?}&tweek={?$day_schedule.tweek?}&parent=periods' onclick="wopen('','scheadd_{?$day_schedule.tweek?}', 800, 600)" target="scheadd_{?$day_schedule.tweek?}">{?t?}добавить  занятие{?/t?}</a></b>&nbsp;
     </td>
    </tr>
    <tr>
