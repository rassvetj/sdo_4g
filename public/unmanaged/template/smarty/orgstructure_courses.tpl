...
<tr>
    <th colspan=99>{?t?}Назначение курсов{?/t?}</th>
</tr>
{?if $items?}
    <tr>
    ...
    {?foreach from=$items item=persons?}
        {?if $persons?}
            ...
            {?foreach name="persons" from=$persons item=person?}
                ...
                {?/if?}
                <tr>
                    <td><input id="{?$person->attributes.owner_soid?}_{?$smarty.foreach.persons.iteration?}" type="checkbox" name="items[]" value="{?$person->attributes.mid?}" checked></td>
                    <td width=99%>{?$person->attributes.person|escape?}</td>
                </tr>
            {?/foreach?}
            </table>
            <br>
    ...
    {?/foreach?}
        ...
        {?if $courses?}
            <table width=100% class=main cellspacing=0>
            <tr><th>{?t?}Курсы{?/t?}</th></tr>
            {?foreach from=$courses key=id item=course?}
                <tr><td><input type="checkbox" name="courses[]" value="{?$id?}" checked> {?$course|escape?}</td></tr>
            {?/foreach?}
            </table>
        {?/if?}
        </td>
...
<tr>
    <td align=right>
        {?t?}Выполнить: {?/t?}
        <select name="action">
            <option value="1"> {?t?}Назначить курсы{?/t?}</option>
            <option value="2"> {?t?}Удалить с курсов{?/t?}</option>
        </select>
    </td>
</tr>
</table>
