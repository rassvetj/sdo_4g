...
$GLOBALS['controller']->setHeader(_('Курсы'));
...
if (isset($_POST['action']) && is_array($_POST['items']) && is_array($_POST['courses'])) {
	...
	foreach($items as $mid) {
		foreach($courses as $cid) {
			if ($_POST['action'] == 1) {
	            CCourseAdaptor::assign($cid, $mid);
                CChainLog::email($cid, $mid);
			}
			
			if ($_POST['action'] == 2) {
			    CCourseAdaptor::drop($cid, $mid);
			}
		}
	}

	$msg = _('Пользователи успешно удалены с курсов');
	if ($_POST['action'] == 1) { 
	   $msg = _('Курсы успешно назначены пользователям');
	}
...
echo $smarty->fetch('orgstructure_courses.tpl');
