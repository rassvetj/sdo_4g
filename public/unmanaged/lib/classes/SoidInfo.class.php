...
class CSoidInfoView {
    ...
    function fetch(&$arr) {
    	...
    	return $tpl->fetch('soid_info.tpl');
    }
...
class CSoidInfoModel {    
    ...
    function execute() {
        if ($this->soid) {
            $this->position = new CSoidInfoPosition();
            $this->position->init($this->_get_info());
        }
    }
    
    function _get_info() {
        $sql = "SELECT structure_of_organ.*, People.*, specializations.name as specialization_name
                FROM structure_of_organ 
                LEFT JOIN People ON (People.MID=structure_of_organ.mid)
                LEFT JOIN specializations ON (structure_of_organ.specialization=specializations.spid)
