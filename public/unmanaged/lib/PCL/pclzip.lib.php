...
//
// --------------------------------------------------------------------------------
// $Id: pclzip.lib.php,v 1.1 2008/06/26 07:35:00 \lex Exp $
// --------------------------------------------------------------------------------
...
// TBC : Removed    $p_header['stored_filename'] = $v_stored_filename;
    ...
    // ----- Look for virtual file
      ...
      $p_header['external'] = 0x00000000;
      $p_header['size'] = strBytes($p_filedescr['content']);
    }
    ...
    if (isset($p_filedescr['comment'])) {
      $p_header['comment_len'] = strBytes($p_filedescr['comment']);
    ...
    if (strBytes($p_header['stored_filename']) > 0xFF) {
...
//      if (is_file($p_filename))
      if (   ($p_filedescr['type'] == 'file')
        ...
        // ----- Look for normal compression
          ...
          // ----- Set header parameters
          $p_header['compressed_size'] = strBytes($v_content);
  ...
  function privWriteFileHeader(&$p_header)
  {
    ...
    $v_binary_data = pack("VvvvvvVVVvv", 0x04034b50,
                          ...
                          $p_header['size'],
                          strBytes($p_header['stored_filename']),
                          $p_header['extra_len']);
    ...
    if (strBytes($p_header['stored_filename']) != 0)
    {
      fputs($this->zip_fd, $p_header['stored_filename'], strBytes($p_header['stored_filename']));
    }
  ...
  function privWriteCentralHeader($p_nb_entries, $p_size, $p_offset, $p_comment)
  {
    ...
    $v_binary_data = pack("VvvvvVVv", 0x06054b50, 0, 0, $p_nb_entries,
                          $p_nb_entries, $p_size,
                          $p_offset, strBytes($p_comment));

    // ----- Write the 22 bytes of the header in the zip file
    fputs($this->zip_fd, $v_binary_data, 22);

    // ----- Write the variable fields
    if (strBytes($p_comment) != 0)
    {
      fputs($this->zip_fd, $p_comment, strBytes($p_comment));
    }
  ...
  function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
  {
      if (stristr(php_uname(), 'windows')) {
          //$p_entry['filename'] = iconv('UTF-8', 'Windows-1251', $p_entry['filename']);
      }
      //$p_entry['filename'] = to_translit($p_entry['filename']);
...
//      if ($p_entry['compressed_size'] == $p_entry['size'])
  ...
  function privReadCentralFileHeader(&$p_header)
  {
    ...
    if (strBytes($v_binary_data) != 42)
    {
      ...
      // ----- Error log
      PclZip::privErrorLog(PCLZIP_ERR_BAD_FORMAT, "Invalid block size : ".strlen($v_binary_data)."\n".$v_binary_data);
//die("Invalid block size : ".strlen($v_binary_data)."\n".$v_binary_data);
      // ----- Return
      //--(MAGIC-PclTrace)--//PclTraceFctEnd(__FILE__, __LINE__, PclZip::errorCode(), PclZip::errorInfo());
      return PclZip::errorCode();
    }
  ...
  // --------------------------------------------------------------------------------
    ...
    {
      if (!function_exists('mb_strlen') && (ini_get('mbstring.func_overload') != 7)) {
          return strlen($str);
      }
