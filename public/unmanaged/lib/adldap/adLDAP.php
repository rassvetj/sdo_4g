...
class adLDAP {
		...
		if ($this->_ad_username!=NULL && $this->_ad_password!=NULL){		    
			...
			if (!$this->_bind){
				if ($this->_use_ssl){
					...
					$this->errors[] = _("FATAL: AD bind failed. Either the LDAPS connection failed or the login credentials are incorrect."); return false;
				} else {
					$this->errors[] = _("FATAL: AD bind failed. Check the login credentials."); return false;
				}
			}
		}
	...
	function authenticate($username,$password,$prevent_rebind=false){
		...
		if ($this->_ad_username!=NULL && !$prevent_rebind){
			$this->_bind = @ldap_bind($this->_conn,$this->_ad_username.$this->_account_suffix,$this->_ad_password);
			if (!$this->_bind){ $this->errors[] = _("FATAL: AD rebind failed."); return false; } //this should never happen in theory
		}
	...
	function user_create($attributes){
		...
		if (array_key_exists("password",$attributes) && !$this->_use_ssl){ $this->errors[] = _("FATAL: SSL must be configured on your webserver and enabled in the class to set passwords."); return false; }
	...
	function user_modify($username,$attributes){
		...
		if (array_key_exists("password",$attributes) && !$this->_use_ssl){ $this->errors[] = ("FATAL: SSL must be configured on your webserver and enabled in the class to set passwords."); return false; }
	...
	function user_password($username,$password){
		...
		if (!$this->_bind){ return (false); }
		if (!$this->_use_ssl){ $this->errors[] = _("FATAL: SSL must be configured on your webserver and enabled in the class to set passwords."); return false; }
	...
	function all_users($include_desc = false, $search = "*", $sorted = true, $dn = '', $filter=''){
		if (!$this->_bind){ return (false); }
		
		if (!empty($filter)) $filter = iconv($GLOBALS['controller']->lang_controller->lang_current->encoding,'UTF-8',$filter);	
		
		//perform the search and grab all their details
		$filter = "(&(objectClass=user)(samaccounttype=". ADLDAP_NORMAL_ACCOUNT .")(objectCategory=person)(cn=".$search.")$filter)";
		$fields=array("samaccountname","displayname","sn","givenName","mail","company");
		if (empty($dn)) $dn = $this->_base_dn;
		$sr=ldap_search($this->_conn,$this->_base_dn,$filter,$fields);
		$entries = ldap_get_entries($this->_conn, $sr);
		$users_array = array();
		for ($i=0; $i<$entries["count"]; $i++){
		    if ($include_desc) {
				$users_array[ $entries[$i]["samaccountname"][0] ] = $entries[$i];
...
*/			} else {
	...
	function all_groups($include_desc = false, $search = "*", $sorted = true){
		...
		$fields=array("samaccountname","description","destingushedname");
		$sr=ldap_search($this->_conn,$this->_base_dn,$filter,$fields);
		$entries = ldap_get_entries($this->_conn, $sr);
...
*/			
			if ($include_desc) {
				$groups_array[ $entries[$i]["samaccountname"][0] ] = $entries[$i];
			} else {
