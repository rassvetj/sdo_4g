...
class Net_IPv4_Custom extends Net_IPv4 
{
	function ipInKnownNetworks($strIp)
	{
		if ($ip = Net_IPv4::parseAddress($strIp)) {
			for ($i = 0; $i <= count($GLOBALS['Net_IPv4_Known_Networks']); $i++) {
				$ipBegin = Net_IPv4::parseAddress($GLOBALS['Net_IPv4_Known_Networks'][$i]['begin']);
				$ipEnd = Net_IPv4::parseAddress($GLOBALS['Net_IPv4_Known_Networks'][$i]['end']);
				if (($ipBegin->long <= $ip->long) && ($ipEnd->long >= $ip->long)) return true;
			}
		}
		return false;
	}
