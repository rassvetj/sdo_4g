<?php
// CVS: $Id: Pager_Wrapper.php,v 1.1.2.2 2009/10/29 08:34:12 cvsup Exp $
//
// Pager_Wrapper
// -------------
//
...
function Pager_Wrapper_Adodb(&$db, $query, $pager_options = array(), $disabled=false)
{
   if (!array_key_exists('totalItems', $pager_options)) {
        ...
        if ($countQuery = rewriteCountQuery($query)) {
            ...
            $totalItems = (int)$res->RecordCount();
            //$res->free();
        }
    ...
    $res = ($disabled)
        ? $db->SelectLimit($query, $totalItems, 0)
