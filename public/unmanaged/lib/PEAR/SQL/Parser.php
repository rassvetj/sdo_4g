...
// +----------------------------------------------------------------------+
//
// $Id: Parser.php,v 1.4.2.2 2009/10/29 08:34:13 cvsup Exp $
//
...
// }}}
        ...
        while (($this->token != ',') && ($this->token != ')') &&
            ...
            switch ($option) {
                case 'constraint':
                    ...
                    break;
                case 'unsigned':
                    $constraintOpts = array('type'=>'unsigned',
                                            'value'=>true);
                    break;
                case 'auto_increment':
    ...
    function parseFieldList()
    {
        ...
        while (1) {
            ...
            if ($this->token == 'ident') {
            ...
            } elseif (($this->token == 'primary') || ($this->token == 'key') || ($this->token == 'unique')) {
                //ignoring keys
                $key = strtoupper($this->token).' ';
                while ($this->token != ')') {
                    $this->getTok();
                ...
                if (!empty($key)) $this->keys[] = $key;

                $this->getTok();
                if ($this->token == ')') return $fields;
                $boolSkip = true;
            } elseif ($this->token == ')') {
                return $fields;
            } else {
            ...
            if (!$boolSkip) {
                ...
                if ($this->token == '(') {
                    ...
                    switch ($fields[$name]['type']) {
                        ...
                        case 'integer': case 'int': case 'timestamp':
                            if (sizeof($values) != 1) {
...
//                    $tree['column_names'] = array_keys($fields);
            ...
            case "database":
                $this->getTok();
                //по-хорошему здесь надо проверять существование
                //базы но у нас название базы прописано в 1.php что тогда проверять???
                
                //$tree = array('command' => 'create_database');
            break;
            default:
    ...
    function parseInsert() {
        ...
        if ($this->token == 'into') {
            ...
            do {
                if (($this->token == 'values') || ($this->token == ',')) {
                    $this->getTok();
                    $results = $this->getParams($values, $types);
                    if (PEAR::isError($results)) {
                        return $results;
            ...
            while ($this->token != 'from') {
                if ($this->token == 'ident') {
                    ...
                    $this->getTok();
                        ...
                        return $this->raiseError('Unexpected token1 "'.$this->token.'"');
                }
            }
        } else {
