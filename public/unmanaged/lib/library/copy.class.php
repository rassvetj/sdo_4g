...
class CCopy {
    ...
    function assign($mid,$start,$stop,$number=1,$type=0) {
        
        if (($mid>0) && ($this->bid>0)) {
            
            if ($this->countCopies()>0) {
                
                $sql = "INSERT INTO library_assign (bid,mid,start,stop,number,type)
                        VALUES ('".(int) $this->bid."','".(int)$mid."','".$start."','".$stop."','".(int) $number."', '".(int) $type."')";
                $res = sql($sql); 
                $this->msg = _("Издание успешно зарезервировано");                       
                
            } else $this->msg = _("Нет свободных экземпляров издания");
    ...
    function delReserv($bid, $mid) {
        return sql("DELETE FROM library_assign WHERE bid = '".(int) $bid."' AND type = '1' AND mid = '".(int) $mid."'");
    }
