...
class CCategory {
    ...
    function get_categories_select($name='categories', $size=1, $width="150px", $multiple='', $all=false) {
        ...
        foreach($cats as $v) {
            ...
            $ret .= "<option label=\"".str_repeat('--',$level)." ".$v['name']."\" value='{$v['catid']}'> ".str_repeat('--',$level)." {$v['name']}</option>";
            $prevLevel = $level;
        }
        $ret .= "</select>";
        return $ret;       
