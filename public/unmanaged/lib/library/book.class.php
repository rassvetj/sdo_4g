...
class CBook {
    ...
    */
    function CBook($data) {
        ...
        $this->title = isset($data['title']) ? trim(strip_tags($data['title'])) : 'Unknown title';
    ...
    */
    function addItem() {
        ...
        $sql = "INSERT INTO library 
                ...
                location,metadata,need_access_level,upload_date,is_active_version,type,is_package,quantity) 
                VALUES 
    ...
    */
    ...
    function getItem($bid) {
        ...
        if ($bid>0) {
            ...
            if (sqlrows($res)) {
                $r = sqlget($res);
                $ret = $r;
                $ret['is_reserved'] = CBook::isReserved($ret['bid'], $_SESSION['s']['mid']);
    ...
    function uploadMaterial($file) {
        ...
        $destPath = $wwf."/library/".(int) $this->bid.'/';
    ...
    function isReserved($bid, $mid) {        
        $sql = "SELECT assid FROM library_assign WHERE bid = '$bid' AND mid = '$mid' AND type = '1'";
        $res = sql($sql);
