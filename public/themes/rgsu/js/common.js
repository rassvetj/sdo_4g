$( document ).ready(function() {
	...
	// if (document.cookie.split(';').slice(-1)[0] != ' alert=0' || getCookie('alert') != 0) {
		
	if (getCookie('ya_alert') != 1 and 0 == 1) {
	
	// console.log($('.hm-roleswitcher-center').text() == 'Администратор');
	if ($("#news_112").length > 0 && $("#news_152").length > 0) {
		document.cookie = 'ya_alert=1; max-age=86400';
		...
		mytext.id = 'myText';
		mytext.innerText = 'Уважаемые обучающиеся! Просим Вас принять участие в анкетировании по знанию иностранных языков до 15.04.2023г. ';
		...
		mylink.id = 'myText';
		mylink.innerText = 'Ссылка на анкету на Яндекс формы';
		mylink.href = 'https://forms.yandex.ru/u/6426b6845056900f67abb69e/';
		el.append(mylink);
		mytext.append(el);
		agreement.append(mytext);
	...
	document.cookie = 'alert=0; max-age=315360000';
