...
window.gridScriptsLoaded || (function () {
	...
	window.updateGridCssClasses = function ($grid) {
		var gridId = $grid.attr('id')
		  , classes = _.map($grid.find('thead > tr:first > th').get(), function (item) { return ''+item.className; })
...
//				.appendTo($grid);
	...
	$(document).delegate('.els-grid tbody tr', 'mouseup', function (event) {
		...
		if ($(event.target).closest('.div_expand').length) { return; }
		if ($tr.is('.filters_tr')) { return; }
		if (event.which != 1) { return; }
		if (!$tr.is('.els-grid-many-actions')) { return; }
		if ($(event.target).is('.ui-els-icon, input, select, button, a')) { return; }
		$tr.siblings('tr.even, tr.odd')
	...
	function gridSubmitValidate (gridId) {
		var $grid = getGrid(gridId)
		  ...
		  , confirmMessage
		  , selectedAction = $gridActions.val();
	...
	$(document).delegate('.els-grid input.mass-checkbox', 'click', function (event) {
		var $target = $(event.currentTarget)
		  , $grid = $target.closest('.els-grid')
		  , $allCheckboxes
		  , $cb = gridPrevSelectedCheckbox[$grid.attr('id')]
		  , cbState
		  , args
		  , changed
		  , selected;
