/**
 ВНИМАНИЕ!!! Код плагина изменён, обновлять нельзя!!
 */
    ...
    $.fn.fcbkcomplete = function(opt) {
        return this.queue( function() {
            ...
            function createFCBK() {
                ...
                complete = $('<div class="facebook-auto">').width(holder.outerWidth());
            ...
            function addInput(focusme) {
                ...
                holder.click( function() {
                    ...
                    if (feed.length && input.val().length > options.input_min_size) {
                        ...
                        clear_feed(1);
                        complete.children(".default").show().end()
                            .position({
                                my: 'left top',
                                at: 'left bottom',
                                of: holder
            ...
            function bindEvents() {
                ...
                maininput.keydown( function(event) {
                    ...
                    var newsize = (options.input_min_size > maininput.val().length) ? options.input_min_size : (maininput.val().length + 1);
                    maininput.attr("size", newsize).width(parseInt(maininput.css('font-size')) * newsize);
                });

                // для запятой отдельно обрабатываем keypress, т.к. в keydown "," = "б"
                maininput.unbind("keypress");
                maininput.keypress( function(event) {

                    if ((event.charCode == _key.comma) && checkFocusOn()) {
                        var option = focuson;
                        addItem(option.text(), option.attr("rel"), 0, 0, 1);
                        return _preventDefault(event);
                    }
                    ...
                    if ((event.charCode == _key.comma) && !checkFocusOn()) {
                        ...
                        if ((options.addoncomma) && options.newel) {
                            focuson = feed.children("li:visible:first");
                            var option = focuson;
                            addItem(option.text(), option.attr("rel"), 0, 0, 1);
                            return _preventDefault(event);
                    ...
                    if (options.cache && json_cache_object.get(etext)) {
                        ...
                        setTimeout( function() {
                            ...
                            $.getJSON(options.json_url, {"tag": xssDisplay(etext)}, function(data) {
                                ...
                                // XXX: don't know why, but we have to swap key & value
