...
class Bvb_Grid_Deploy_Table extends Bvb_Grid implements Bvb_Grid_Deploy_DeployInterface
{
                        ...
                        if ( $this->getInfo("ajax") !== false ) {
                            ...
                            if ( $this->getAlwaysShowOrderArrows() === true ) {
                                ...
                                $grid .= !empty($title['url']) ?
                                    str_replace(array('{{value}}', '{{title}}', '{{class}}'), array("<a href=\"javascript:gridAjax('{$this->getInfo('ajax')}','" . $title['url'] . "') \">" . $title['value'] . $imgFinal . "</a>", $titleName, $title['class']), $this->_temp['table']->titlesLoop()) :
    ...
    protected function _pagination ()
    {
        ...
        ?>
        (function(){paginatorChange('<?php echo "$jsUrl/start{$this->getGridId()}/"; ?>',<?php echo $this->getResultsPerPage()?>,'<?php echo $this->getInfo("ajax")?>')})()
        <?php
...
//        $bTitles .= $this->_buildMassActions();
        ...
        // Добавляем наши фиксированные строки
        /*if($this->_hasFixedRows == true){
            $bGrid.=$this->_fixedRows;
        }*/
...
if ( $useAjax == 1 ) {
    ...
    function gridAjax(ponto,url) {
        ...
        url = url.replace('//', '/');
        url = url.replace('subject/ /', 'subject//');
//[che 05.06.2014 #16600]	//разработчик испугался точек, они действительно разрушают URL, но на то есть url encoding!
	for(var i=0; i<100; i++)
        	url = url.replace('.','%2E');
/*
        url = url.replace('.','-');
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                foreach($this->_filters as $key =>$value) {
                    if (strtolower($value['render']) == 'date') {
...
function urlencode(str) {
    return str.replace(/\+/g,'%2B').replace(/%20/g, '+').replace(/\*/g, '%2A').replace(/\//g, '%2F').replace(/@/g, '%40').replace(/\?/g, '%3F');
...
//        if ($this->newHtmlTable) {
//
//            $gridParams = array(
//                'gridId' => $this->_gridId,
//                'cols' => $this->getFieldsInfo(),
//                'width' => $this->tableWidth
//            );
//
//            $script .= '$(function() {
//                HM.create("hm.core.ui.Grid", '.json_encode($gridParams).');
//            });';
//        }
...
protected function _buildExtraRows ($position)
       ...
       // print_r(htmlspecialchars($end));
        ...
        foreach ( $this->_extraRows as $key => $value ) {
            ...
            foreach ( $value['values'] as $key => $final ) {
                ...
                {
                    if($this->_hasFixedRows){
                        $middle1 .= "<td $colspan $class ><input type='checkbox' name='gridMassActions_" . $this->_gridId . "' id='massCheckBox_" . $this->_gridId . "' class='mass-checkbox' value='" . $final['value'] . "' checked='checked'><span class='mass-fixer'><img src='/images/icons/attention.gif' onclick=\"toggleFix(".$final['value'].", this)\" id=\"toggle_".$final['value']."\"></span></td>".PHP_EOL;
                    }else{
                        $middle1 .= "<td $colspan $class ><input type='checkbox' name='gridMassActions_" . $this->_gridId . "' id='massCheckBox_" . $this->_gridId . "' class='mass-checkbox' value='" . $final['value'] . "' checked='checked'></td>".PHP_EOL;
                    }
    ...
    protected function _buildMassActions ()
    {
        ...
        if (count($this->_subMassActionSelects)) {
			$onChange .= "jQuery('img.multiple_toggle').hide();";
            foreach($this->_subMassActionSelects as $massAction => $subSelect) {
                ...
                $onChange .= "if (this.value == '".$massAction."') {jQuery('#".preg_replace("/\[|\]/","",$subSelect['name'])."').show();jQuery('img.multiple_toggle').show();}";
        ...
        if (count($this->_subMassActionFcbk)) {
            foreach($this->_subMassActionFcbk as $massAction => $subInput) {
                if (!isset($subInputs[$subInput['name']])) {
                    $onChange .= "jQuery('#_fdiv ul.holder').hide();";
                }
                $onChange .= "if (this.value == '".$massAction."') {jQuery('#_fdiv ul.holder').show();}";

                if (!isset($subInputs[$subInput['name']])) {
                    $formSelect .= '<div class="container-fcbk">' . $this->getView()->fcbkComplete($subInput['name'], null, array(
                        'width' => 240,
        ...
        if (count($this->_subMassActionInput)) {
            ...
            $gridSubactionInputCount = 0;

            foreach($this->_subMassActionInput as $massAction => $subInput) {

                $gridSubactionInputId = 'grid_subaction_input_'.$gridSubactionInputCount++;

                if (!isset($subInputs[$subInput['name']])) {
                    $onChange .= "jQuery('#$gridSubactionInputId').hide();";
                    $formSelect .= '<input type="text" name="'.$subInput['name'].'" size="40" id="'.$gridSubactionInputId.'" style="display:none;">';
                    $subInputs[$subInput['name']] = true;
                }
                $onChange .= "if (this.value == '".$massAction."') {jQuery('#$gridSubactionInputId').show();}";
            }
        }
        ...
        if (!$this->_options['ignoreMassActionsIds']) {
        ...
        <strong>" . $this->__('Для') . " <span id='massSelected_" . $this->getGridId() . "' class='selected-count'>0</span> " . $this->__('элементов:') . "</strong></span> " . /*$this->__('Actions') . */" $formSelect <img class='multiple_toggle' src='/images/bullet_toggle_plus.png'/> $formSubmit
        </div></form>";
        //$return .= "</div>";
        //$return .= "</td></tr>";
    ...
    function setMassAction(array $options)
    {
        ...
        if($this->_hasFixedRows) {
            $view = $this->getView();
            $decorator = "<input type='checkbox' name='gridMassActions_".$this->getGridId()."' id='massCheckBox_".$this->getGridId()."' class='mass-checkbox' value='{{{$pk}}}' ><span class='mass-fixer'>".$view->icon('{{fixType}}', _('Фиксирование строки'),"toggleFix({{{$pk}}}, this)", "toggle_{{{$pk}}}")."</span>";
        } elseif ($this->_hasShotCut) {
            $view = $this->getView();
            $decorator = "<input type='checkbox' name='gridMassActions_".$this->getGridId()."' id='massCheckBox_".$this->getGridId()."' class='mass-checkbox' value='{{{$pk}}}' ><span class='mass-fixer'>".$view->icon('{{'.$this->_shotCutField.'}}', _('Фиксирование строки'),"toggleShotCut({{{$pk}}}, this)", "toggle_{{{$pk}}}")."</span>";
        } else {
            $decorator = "<input type='checkbox' name='gridMassActions_".$this->getGridId()."' id='massCheckBox_".$this->getGridId()."' class='mass-checkbox' value='{{{$pk}}}' >";
        }
    ...
    public function getGmailCheckbox()
    {
        ...
        $this->getView()->disableExtendedFile();
        $gridId = $this->getGridId();
        ...
        $this->getView()->enableExtendedFile();
        return $content;
    }
