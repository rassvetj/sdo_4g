...
//
            ...
            if ( $key == 'filters' . $this->getGridId() ) {
                ...
                if(!is_array($param) && $key!== 'masterOrder' . $this->getGridId()){
    ...
    protected function _buildTitles ()
    {
        ...
        foreach ( $this->_extraFields as $key => $value ) {
            if ( $value['position'] == 'left' ) {
                $title = $return[$key] = array('class' => $this->__(isset($value['class']) ? $value['class'] : ''),'type' => 'extraField', 'value' => $this->__(isset($value['title']) ? $value['title'] : $value['name']), 'position' => 'left');
            }
        }
...
//                if ( $noOrder == 1 ) {
        ...
        foreach ( $this->_result as $dados ) {
            ...
            foreach ( $fields as $campos ) {
                ...
                if ( $this->_displayField($fields[$is]) ) {
                    ...
                    $return[$i][] = array('fixType' => isset($dados['fixType']) ? $dados['fixType'] : '', 'class' => $fieldClass . ' ' . $finalClassConditional, 'value' => $new_value, 'field' => $this->_fields[$is], 'style' => $style);
                }
    ...
    public function deploy ()
    {
        ...
        if($this instanceof Bvb_Grid_Deploy_Table){
                ...
                foreach ( $this->_actions as $action )
                {
                  // echo $test = substr($action['url'], 33, 7); 
                   ...
                   //print_r($this->_prepareReplace($this->_actions));
					$confirm = empty($action['confirm']) ? "" : "onClick=\"if (confirm('" . htmlspecialchars($action['confirm']) . "') == false){ return false; }\" ";
                    $actionsDecorator .= "" . "<li><a href =\"" . $action['url'] . "\" " . $confirm . " >" . $action['icon'] . "</a></li>";
        ...
        if (! empty($this->_massActions)) {
            ...
            $event = new sfEvent(null, HM_Extension_ExtensionService::EVENT_FILTER_GRID_MASSACTIONS);
    ...
    protected function flushState()
    {
    	...
    	$default->grid[$page][$this->getGridId()] = null;
    }
    ...
    protected function restoreState()
    {
        ...
        if (isset($default->grid[$page][$this->getGridId()])) {
            ...
            if ($default->grid[$page][$this->getGridId()]['order']) {
                $this->setParam('order'.$this->getGridId(), $default->grid[$page][$this->getGridId()]['order']);
            }
            if ($default->grid[$page][$this->getGridId()]['start']) {
    ...
    public static function getGridSwitcherParamById($gridId)
    {
        ...
        if (isset($default->grid[$page][$gridId])) {
            ...
            if (isset($default->grid[$page][$gridId]['all'])) {
                return $default->grid[$page][$gridId]['all'];
            }
        }
    }
    ...
    protected function saveState()
    {
        ...
        if (null !== $this->_gridSwitcher) {
            $default->grid[$page][$this->getGridId()]['all'] = $request->getParam('all');
        }
    }
    ...
    public static function factory ($defaultClass, $options = array(), $id = '', $classCallbacks = array(), $requestData = false)
    {
        ...
        if (is_array($requestData['_exportTo' . $id])) {
    ...
    public function addAction($url, $params, $icon, $confirm = null)
    {
        $_url = $url;
        if (is_array($url)) {
        ...
        $this->_actions[] = array('url' => $_url, 'icon' => $icon, 'confirm' => $confirm);
    ...
    public function addMassAction($url, $caption, $confirm = null)
    {
    ...
    public function addSubMassActionFcbk($massActionUrl, $fcbkName)
    {
    ...
    public function addSubMassActionInput($massActionUrl, $inputName)
    {
        if (is_string($massActionUrl)) {
            $massActionUrl = array($massActionUrl);
        }
    ...
    public function addSubMassActionSelect($massActionUrl, $selectName, $selectOptions)
    {
        if (is_string($massActionUrl)) {
            $massActionUrl = array($massActionUrl);
        } else {
