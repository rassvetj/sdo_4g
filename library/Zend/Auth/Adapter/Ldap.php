    ...
    public function authenticate()
    {
        ...
        foreach ($this->_options as $name => $options) {
            ...
            try {
				...
				/********************************************************************/
				$LDAP = Zend_Registry::get('config')->ldap->special->options->toArray();
				...
				$isTeacher = false;
				foreach($LDAP as $i){
					$conn = ldap_connect($i['server'], $i['port']); 	
					ldap_set_option($conn, LDAP_OPT_PROTOCOL_VERSION, 3); 
					ldap_set_option($conn, LDAP_OPT_REFERRALS,0);					
					$bb = @ldap_bind($conn, $i['prefix'].'\\'.$u_username, $u_password);
				...
				if($bb === false){ //--пара логин + пароль не подошли. Ищем по логину статус
					$blockedCodes = array(
						'514',
						'546',
						'66050',
					...
					foreach($LDAP as $i){
						...
						ldap_set_option($conn, LDAP_OPT_PROTOCOL_VERSION, 3); 
						ldap_set_option($conn, LDAP_OPT_REFERRALS,0);
						$bb = @ldap_bind($conn, $i['bindDN'], $i['bindPW']);

						$dn_teacher = 'OU=РГСУ,OU=Domain Users,DC=rgsu,DC=local';
						$dn = 'OU=Students,DC=edu,DC=local';
						$filter = "(sAMAccountName=".$u_username.")";
						$attr = array('userAccountControl', 'msds-userpasswordexpirytimecomputed');
						$search = @ldap_search($conn,$dn,$filter,$attr);
						if($search){
							$entries = ldap_get_entries($conn, $search);
						}
					
						if(count($entries) < 1 || $entries['count'] < 1){ 
							$search = @ldap_search($conn,$dn_teacher,$filter,$attr);				
							if($search){
								$entries = ldap_get_entries($conn, $search); 	
							}
							
							if(count($entries) > 0){ //--если дошлю сюда, значит это преподаватель
								$isTeacher = true;
							}	
						}
						...
						if(count($entries) > 0 && $entries['count'] > 0){
							...
							if(in_array($entries[0]['useraccountcontrol'][0], $blockedCodes)){																		
								if($isTeacher){
									$code = Zend_Auth_Result::FAILURE_TEACHERS_BLOCKED;	
								} else {
									$code = Zend_Auth_Result::FAILURE_USER_BLOCKED;	
								}									
								
								$messages[0] = '';
								$messages[1] = '';
								$messages[] = 'User is blocked';
								return new Zend_Auth_Result($code, '', $messages);									
							
							} elseif(empty($entries[0]['msds-userpasswordexpirytimecomputed'][0])) {
								$code = Zend_Auth_Result::PASSWORD_EXPIRED;
								$messages[0] = '';
								$messages[1] = '';
								...
								$messages[0] = '';
								$messages[1] = '';
								$messages[] = 'Login or password is incorrect';
								return new Zend_Auth_Result($code, '', $messages);
							}														
						}
					}	
